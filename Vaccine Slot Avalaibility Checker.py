...
result_box_D1D2=Text(app,height=20,width=7,bg='#293241',fg='#ecfcff',relief=FLAT,font='verdana 10')
...
radio_location=Radiobutton(app,text='Current Location',bg=top_right_frame_bg,variable=curr_loc_var,value=curr_loc_var,command=fill_pincode)
...
def search_vaccine_avl():
    ...
    try:
        if len(resp_JSON['sessions'])==0:
            messagebox.showinfo("INFO","Vaccine is not available yet for the given date")
...
#   buttons
search_vaccine_image = PhotoImage(file= r"C:\Users\ABHIRAJ\search-icon.png")
search_vaccine_btn = Button(app, image=search_vaccine_image,command=search_vaccine_avl, bg= top_right_frame_bg, relief= RAISED)
